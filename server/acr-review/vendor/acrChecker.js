...
var ACR_WHITE_FIELDS = {"Service #": 1, "CACC/ACS": 1, "Call Number": 1, "Call Date": 1, "Date of birth": 1, "Health Insurance Number": 1, "Version Code": 1, "Date of Occurrence": 1, "Time of Occurrence": 1, "DNR confirmation number": 1, "Location 1": 1, "Type 1": 1, "Mechanism 1": 1, "Location 2": 1, "Type 2": 1, "Mechanism 2": 1, "Location 3": 1, "Type 3": 1, "Mechanism 3": 1, "Arrest witnessed date": 1, "Arrest witnessed Time": 1, "CPR started Date": 1, "CPR Started Time": 1, "First shock Date": 1, "First Shock Time": 1, "Treatment Row 1 - Line": 1, "Treatment Row 2 - Line": 1, "Treatment Row 3 - Line": 1, "Treatment Row 4 - Line": 1, "Treatment Row 5 - Line": 1, "Treatment Row 6 - Line": 1, "Treatment Row 7 - Line": 1, "Treatment Row 8 - Line": 1, "Treatment Row 9 - Line": 1, "Treatment Row 10 - Line": 1, "Treatment Row 11 - Line": 1, "Treatment Row 12 - Line": 1, "Treatment Row 13 - Line": 1, "Treatment Row 14 - Line": 1, "Treatment Row 15 - Line": 1, "Treatment Row 16 - Line": 1, "Treatment Row 17 - Line": 1, "Treatment Row 18 - Line": 1, "Treatment Row 19 - Line": 1, "Treatment Row 20 - Line": 1, "Treatment Row 21 - Line": 1, "Treatment Row 22 - Line": 1, "Treatment Row 23 - Line": 1, "Treatment Row 24 - Line": 1, "Treatment Row 25 - Line": 1, "Treatment Row 26 - Line": 1, "Date5_TOR": 1, "Time5_TOR": 1, "Vehicle Number": 1, "Station": 1, "Status": 1, "Hospital Number": 1, "UTM Code": 1, "Dispatch #": 1, "Return #": 1, "Patient #": 1, "Sequence": 1, "Base Hospital Number": 1, "Patch Log Number": 1, "Call Received HH  MM  SS": 1, "Crew Notified HH  MM  SS": 1, "Crew Mobile HH  MM  SS": 1, "Arrive Scene HH  MM  SS": 1, "Patient Contact HH  MM  SS": 1, "Depart Scene HH  MM  SS": 1, "Arrive Destination HH  MM  SS": 1, "TOC  HH  MM  SS": 1, "Crew 1 Number (Attending)": 1, "Crew 2 Number": 1, "Crew 3 Number": 1, "Crew 4 Number": 1, "Date of ACR Completion": 1, "Time of ACR Completion HH  MM  SS": 1, "Treatment Row 27 - Line": 1, "Treatment Row 28 - Line": 1, "Treatment Row 29 - Line": 1, "Treatment Row 30 - Line": 1, "Treatment Row 31 - Line": 1, "Treatment Row 32 - Line": 1, "Treatment Row 33 - Line": 1, "Treatment Row 34 - Line": 1, "Treatment Row 35 - Line": 1, "Treatment Row 36 - Line": 1, "Treatment Row 37 - Line": 1, "Treatment Row 38 - Line": 1, "Treatment Row 39 - Line": 1, "Treatment Row 40 - Line": 1, "Treatment Row 41 - Line": 1, "Treatment Row 42 - Line": 1, "Treatment Row 43 - Line": 1, "Treatment Row 44 - Line": 1, "Treatment Row 45 - Line": 1, "Treatment Row 46 - Line": 1, "Treatment Row 47 - Line": 1, "Treatment Row 48 - Line": 1, "Treatment Row 49 - Line": 1, "Treatment Row 50 - Line": 1, "Treatment Row 51 - Line": 1, "Treatment Row 52 - Line": 1, "Patient or SDM Signature Time": 1, "Patient or SDM Signature Date": 1, "Attending Paramedic Signature Time": 1, "Attending Paramedic Signature Date": 1, "Non-paramedic Witness Signature Time": 1, "Non-paramedic Witness Signature Date": 1};
var ACR_ALWAYS_READONLY = {"Tools Title": 1, "Submitted Stamp": 1, "Full Line Hint": 1, "Cont Call Number": 1, "Cont Page Number": 1, "Cont Total Pages": 1, "Full Line Hint C": 1, "Review - Call Number": 1, "Review - Call Intervals": 1, "Review - Vitals Trend": 1, "Review - Check Results": 1, "Treatment Page Number": 1, "Treatment Total Pages": 1};
...
var ACR_ROWS = 52;
var ACR_NARR_COLS = ["Dose/Unit", "Route", "Pulse", "Resp", "BP", "Temp", "Reading/Code", "SpO2", "EtCO2", "GCS", "Pupil R", "Pupil L", "Pain"];
var ACR_EVENTS = [
...
function acrRow(doc, i) {
  var r = { i: i, time: acrV(doc, acrRowName(i, "Time")), code: acrV(doc, acrRowName(i, "Procedure Code")),
            crew: acrV(doc, acrRowName(i, "Crew Member Number")), line: acrV(doc, acrRowName(i, "Line")),
            fullLine: acrOn(doc, acrRowName(i, "Full Line")) };
...
// ------------------------------------------------------------------ full-line toggle
function acrToggleLine(doc, i) {
  var p = "Treatment Row " + i + " - ", box = doc.getField(p + "Full Line"), line = doc.getField(p + "Line"), k;
  var on = box.valueAsString !== "Off";
  if (on) {
    var used = false;
    for (k = 0; k < ACR_NARR_COLS.length; k++) if (acrV(doc, p + ACR_NARR_COLS[k]) !== "") used = true;
    if (used) { app.alert("This row already has entries between Dose/Unit and Pain Scale. Clear them first, then tick the box to write across the row."); box.value = "Off"; return; }
    for (k = 0; k < ACR_NARR_COLS.length; k++) doc.getField(p + ACR_NARR_COLS[k]).display = display.hidden;
    line.display = display.visible; line.setFocus();
  } else {
    if (acrV(doc, p + "Line") !== "") { app.alert("This row has narrative text. Clear it first, then untick the box to go back to the columns."); box.value = "Yes"; return; }
    line.display = display.hidden;
    for (k = 0; k < ACR_NARR_COLS.length; k++) doc.getField(p + ACR_NARR_COLS[k]).display = display.visible;
  }
}
...
function acrCheckIssues(doc) {
  ...
  for (k = 0; k < rows.length; k++) {
    ...
    if (acrIsMed(r.code) || acrIsO2(r.code)) {
      ...
      if (r.fullLine || (r.line && !r["Dose/Unit"] && !r["Route"])) {
        if (!r.line) add("Row " + r.i + " (" + what + ") is set to write across, but the line is empty.", [acrRowName(r.i, "Line")]);
        else {
          if (!ACR_DOSE_IN_LINE.test(r.line)) add("Row " + r.i + " (" + what + ") is written across, but the line has no " + dname + " with a unit.", [acrRowName(r.i, "Line")]);
          if (!acrIsO2(r.code) && !acrLineRoute(r.line)) add("Row " + r.i + " (" + what + ") is written across, but the line has no route code (PO, SL, IM, NB and so on).", [acrRowName(r.i, "Line")]);
        }
      } else {
