...
async function runCase(testCase, index) {
  ...
  try {
    ...
    const picked = pickTwelveLeadPattern(ecgType, rhythmInterp, twelveLeadFindings, fifteenLeadFindings);
    const match = picked === intended;
...
async function main() {
  ...
  for (const r of results) {
    ...
    console.log(
      `${tag} ${matchFlag} | intended=${r.intended.padEnd(20)} picked=${r.picked.padEnd(20)}` +
      ` | ecgType=${String(r.ecgType).padEnd(10)} 12L=${r.has12LeadFindings ? 'Y' : 'N'} 15L=${r.has15LeadFindings ? 'Y' : 'N'}` +
      ` | ${r.label} (${r.durationMs}ms)`
    );
    if (!r.match) {
      console.log(`       findings: "${r.twelveLeadFindingsSnippet}"`);
  ...
  if (missed.length) {
    ...
    missed.forEach(r => console.log(`    - ${r.label}: intended=${r.intended}, picked=${r.picked}`));
  }
  console.log('─────────────────────────────────────────────────────────\n');
